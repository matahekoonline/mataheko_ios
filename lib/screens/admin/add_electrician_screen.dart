...
/// Admin utility screen for directly adding an Electrician entry (as
/// opposed to a provider self-registering through BioDataScreen). Mirrors
/// the Add Mechanic form. New entries still start isPending: true and
/// need one tap on "Approve" back on ElectriciansScreen before they show
/// up publicly -- there is deliberately no auto-approve shortcut here.
class AddElectricianScreen extends StatefulWidget {
  const AddElectricianScreen({super.key});

  @override
  State<AddElectricianScreen> createState() => _AddElectricianScreenState();
}

class _AddElectricianScreenState extends State<AddElectricianScreen> {
  ...
  Future<void> _submit() async {
    ...
    try {
      ...
      await AuthService.instance.addElectricianByAdmin(
      ...
      ScaffoldMessenger.of(context).showSnackBar(
        const SnackBar(content: Text('Electrician added — approve them from the list to make them public.')),
      );
      Navigator.pop(context, true);
    } catch (e) {
      // ignore: avoid_print
      print('[AddElectricianScreen] Save failed: $e');
      if (!mounted) return;
      setState(() => _error = 'Failed to save electrician: $e');
    } finally {
  ...
  Widget build(BuildContext context) {
    return Scaffold(
      appBar: AppBar(title: const Text('Add Electrician')),
      body: SafeArea(
          ...
          child: Form(
            ...
            child: Column(
              ...
              children: [
                ...
                const SizedBox(height: 20),
                _chipSection('Property Types Serviced', electricianPropertyTypeOptions, _selectedPropertyTypes),
                const SizedBox(height: 20),
                _chipSection('Services Offered', electricalServiceOptions, _selectedServices),
                const SizedBox(height: 12),
                SwitchListTile(
