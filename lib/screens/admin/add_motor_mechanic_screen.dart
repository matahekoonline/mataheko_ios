...
class AddMotorMechanicScreen extends StatefulWidget {
  const AddMotorMechanicScreen({super.key});

  @override
  State<AddMotorMechanicScreen> createState() => _AddMotorMechanicScreenState();
}

class _AddMotorMechanicScreenState extends State<AddMotorMechanicScreen> {
  ...
  Future<void> _submit() async {
    ...
    if (_mechanicPhoto == null) {
      setState(() => _error = 'Please add a photo of the motor mechanic.');
      return;
    }
    ...
    try {
      final docRef = FirebaseFirestore.instance.collection('motorMechanics').doc();
      ...
      final mechanic = MotorMechanic(
        ...
        rating: 0.0,
        reviewCount: 0,
        isApproved: true, // admin-added motor mechanics are approved immediately
        isPending: false,
      ...
      if (mounted) {
        ScaffoldMessenger.of(context).showSnackBar(const SnackBar(content: Text('Motor mechanic added')));
        Navigator.pop(context, true);
      }
    } catch (e) {
      setState(() => _error = 'Failed to save motor mechanic. Please try again.');
    } finally {
  ...
  Widget build(BuildContext context) {
    return Scaffold(
      appBar: AppBar(title: const Text('Add Motor Mechanic')),
      body: SafeArea(
          ...
          child: Form(
            ...
            child: Column(
              ...
              children: [
                ...
                const Center(
                  child: Text('Motor mechanic photo (shown to app users)', style: TextStyle(fontSize: 12, color: Colors.grey)),
                ),
                ...
                TextFormField(
                  ...
                  decoration: const InputDecoration(labelText: 'Motor Mechanic Name', border: OutlineInputBorder()),
                ...
                const SizedBox(height: 20),
                _chipSection('Vehicle Types Serviced', motorVehicleTypeOptions, _selectedVehicleTypes),
                const SizedBox(height: 20),
                _chipSection('Brand Specialties (optional)', motorBrandSpecialtyOptions, _selectedBrands),
                const SizedBox(height: 20),
                _chipSection('Services Offered', motorServiceOptions, _selectedServices),
                ...
                ElevatedButton(
                  ...
                  child: _loading
                      ...
                      : const Text('Save Motor Mechanic'),
                ),
              ],
            ),
          ),
