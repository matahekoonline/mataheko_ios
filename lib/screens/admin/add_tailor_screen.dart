...
class AddTailorScreen extends StatefulWidget {
  const AddTailorScreen({super.key});
...
class _AddTailorScreenState extends State<AddTailorScreen> {
  ...
  Future<void> _submit() async {
    ...
    if (_selectedGarmentTypes.isEmpty) {
      setState(() => _error = 'Select at least one garment type.');
      return;
    }
    if (_tailorPhoto == null) {
      setState(() => _error = 'Please add a photo of the tailor.');
      return;
    }
    ...
    try {
      final docRef = FirebaseFirestore.instance.collection('tailors').doc();
      ...
      final tailor = Tailor(
        ...
        rating: 0.0,
        reviewCount: 0,
        // Admin-added tailors still go through the same one-tap approval
      ...
      await docRef.set({
        ...tailor.toMap(),
        // TailorsScreen queries .orderBy('createdAt', descending: true) --
        // Firestore silently EXCLUDES any doc missing that field from an
        // orderBy query (no error, it just never shows up). Tailor.toMap()
        // doesn't set this itself, so it has to be added here explicitly,
        // same as AuthService.registerAsTailor already does for
        // self-registered tailors.
        'createdAt': FieldValue.serverTimestamp(),
      });

      if (mounted) {
        ScaffoldMessenger.of(context).showSnackBar(
          const SnackBar(content: Text('Tailor added — approve them from the list to make them public.')),
        );
        Navigator.pop(context, true);
      }
    } catch (e) {
      setState(() => _error = 'Failed to save tailor. Please try again.');
    } finally {
  ...
  Widget build(BuildContext context) {
    return Scaffold(
      appBar: AppBar(title: const Text('Add Tailor')),
      body: SafeArea(
          ...
          child: Form(
            ...
            child: Column(
              ...
              children: [
                Center(
                  child: GestureDetector(
                    ...
                    child: CircleAvatar(
                      ...
                      backgroundImage: _tailorPhoto != null ? FileImage(_tailorPhoto!) : null,
                      child: _tailorPhoto == null
                          ? Icon(Icons.add_a_photo, color: Colors.purple[700], size: 28)
                          : null,
                    ),
                  ),
                ),
                ...
                const Center(
                  child: Text('Tailor photo (shown to app users)', style: TextStyle(fontSize: 12, color: Colors.grey)),
                ),
                ...
                TextFormField(
                  ...
                  decoration: const InputDecoration(labelText: 'Tailor Name', border: OutlineInputBorder()),
                ...
                const SizedBox(height: 20),
                _chipSection('Garment Types Serviced', garmentCategoryOptions, _selectedGarmentTypes),
                const SizedBox(height: 20),
                _chipSection('Fabric Specialties (optional)', fabricSpecialtyOptions, _selectedFabrics),
                const SizedBox(height: 20),
                _chipSection('Services Offered', tailoringServiceOptions, _selectedServices),
